...
def train_epoch_adjoint_diff(cur_epoch, model, args,
    ...
    total_sample = 0.
    total_loss = 0.
    num_weights, num_hypers = sum(
        p.numel() for p in model.parameters()), 2*((num_classes-1)//group_size)+1
        ...
        if is_up:
            ...
            if cur_iter % ARCH_INTERVAL == 0:
                ...
                for _ in range(ARCH_VAL_SAMPLE):
                    ...
                    up_loss = up_criterion(
                        up_preds, up_targets, up_params, group_size=group_size)
                ...
                for _ in range(ARCH_EULAR_STEP):
                    ...
                    for _ in range(ARCH_TRAIN_SAMPLE):
                        ...
                        low_loss = low_criterion(
                            low_preds, low_targets_alt, low_params, group_size=group_size)
                    ...
                    vector_jacobiao_pro=gather_flat_grad(
                        grad(d_train_loss_d_theta, get_trainable_hyper_params(up_params)+list(model_tilde.parameters()), grad_outputs=d_val_loss_d_theta.view(-1), retain_graph=True, allow_unused=True))
                ...
                up_optimizer.zero_grad()
                assign_hyper_gradient(up_params, hyber_grad, num_classes)
                up_optimizer.step()

        low_preds = model(low_data)

        # Compute the loss
        loss = low_criterion(low_preds, low_targets,
                             low_params, group_size=group_size)
...
def train_epoch_adjoint_diff_momentum(cur_epoch, model, args,
                              low_loader, low_criterion, low_optimizer, low_params=None,
                              up_loader=None, up_optimizer=None, up_criterion=None, up_params=None):
    """Performs one epoch of ODE bilevel optimization."""
    num_classes = args["num_classes"]
...
def eval_epoch(data_loader, model, criterion, cur_epoch, text, args, params=None):
    ...
    for cur_iter, (data, targets) in enumerate(data_loader):
        ...
        mb_size = data.size(0)
        loss += criterion(logits, targets, params,
                          group_size=group_size).item()*mb_size
